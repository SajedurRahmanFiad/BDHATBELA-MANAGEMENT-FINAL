...
```powershell
npm run deploy:cpanel:prepare:admin
```

This creates:

- `deploy/cpanel-admin-package/`
...
Upload `deploy/cpanel-admin-package.zip` into your cPanel home directory and extract it.
...
- the cPanel API entry file is `admin.bdhatbela.com/api/index.php`
...
- if your cPanel subdomain uses a different document-root folder than `admin.bdhatbela.com`, generate a package with a different `-DocumentRootFolder` value
