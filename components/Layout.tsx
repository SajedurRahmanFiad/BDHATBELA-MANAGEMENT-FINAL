        ...
        <div className="flex flex-col h-full">
          <div className="p-8">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className={`text-xl font-black ${theme.colors.text.primary} tracking-tight leading-none`}>{db.settings.company.name}</h1>
          ...
          <div className={`p-6 border-t ${theme.colors.border.primary}`}>
            <button 
              onClick={handleLogout}
              className={`flex items-center gap-3 w-full px-4 py-3 ${theme.colors.danger.text} hover:${theme.colors.danger[50]} ${theme.radius.md} ${theme.transitions.normal} font-bold text-sm`}
            >
              {ICONS.LogOut}
              Logout
            </button>
      ...
      <div className="flex-1 flex flex-col min-w-0 h-screen overflow-hidden">
        <header className={`flex-shrink-0 sticky top-0 z-40 ${theme.colors.bg.primary}/80 backdrop-blur-lg border-b ${theme.colors.border.primary} px-6 h-20 flex items-center justify-between`}>
          ...
          <div className="flex items-center gap-4">
            ...
            <div className={`flex items-center gap-3 pl-4 border-l ${theme.colors.border.primary}`}>
              ...
              <img src={user.image || `https://ui-avatars.com/api/?name=${user.name}&background=0f2f57&color=fff`} alt="Profile" className="w-10 h-10 rounded-xl border-2 border-[#e6f0ff] object-cover shadow-sm" />
            </div>
          </div>
        </header>
        ...
        <main className="flex-1 overflow-y-auto p-6 lg:p-10 animate-in fade-in duration-500 relative">
          ...
          <footer className={`mt-20 py-8 border-t ${theme.colors.border.primary} flex flex-col items-center gap-2`}>
            <p className={`text-sm font-medium ${theme.colors.text.secondary}`}>© {new Date().getFullYear()} {db.settings.company.name}. All rights reserved.</p>
