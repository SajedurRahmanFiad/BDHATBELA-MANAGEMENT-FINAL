...
final class OperationsApi extends BaseService
{
    ...
    public function fetchPayrollSummaries(array $params): array
    {
        ...
        foreach ($orderRows as $row) {
            ...
            if (!$this->isWalletEligibleOrderDate((string) ($row['order_date'] ?? ''), (string) ($row['created_at'] ?? ''))) {
                continue;
            }

            $activityDate = $this->walletEligibleLocalDate((string) ($row['order_date'] ?? ''), (string) ($row['created_at'] ?? ''));
            if ($activityDate === '' || $activityDate < $periodStart || $activityDate > $periodEnd) {
                continue;
            }
    ...
    {
        $currentUser = $this->currentUser();
        $sql = 'SELECT * FROM employee_wallet_balances';
        $bindings = [];
        if (!$this->hasAdminAccess((string) ($currentUser['role'] ?? ''))) {
            $sql .= ' WHERE employeeId = :employee_id';
            $bindings[':employee_id'] = (string) $currentUser['id'];
        }
        $sql .= ' ORDER BY currentBalance DESC, employeeName ASC';

        $rows = $this->database->fetchAll($sql, $bindings);
        return array_map(fn (array $row): array => $this->mapWalletBalanceCard($row), $rows);
    }
    ...
    public function fetchMyWallet(array $params = []): ?array
    {
        $currentUser = $this->currentUser();
        $row = $this->database->fetchOne(
            'SELECT * FROM employee_wallet_balances WHERE employeeId = :employee_id LIMIT 1',
        ...
        return $this->mapWalletBalanceCard($row);
    }
    ...
    public function payEmployeeWallet(array $params): array
    {
        ...
        return $this->database->transaction(function () use ($currentUser, $employeeId, $amount, $params): array {
            ...
            $walletRow = $this->database->fetchOne(
                'SELECT currentBalance FROM employee_wallet_balances WHERE employeeId = :employee_id LIMIT 1',
                [':employee_id' => $employeeId]
            );
            $walletBalance = (float) ($walletRow['currentBalance'] ?? 0);
