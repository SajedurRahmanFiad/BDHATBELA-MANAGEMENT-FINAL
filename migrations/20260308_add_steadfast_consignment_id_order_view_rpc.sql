-- Add steadfast_consignment_id to orders and wire it through order view + RPC

BEGIN;

-- Ensure both consignment columns exist.
-- carrybee_consignment_id is included for safety on environments that missed earlier schema updates.
ALTER TABLE IF EXISTS orders
  ADD COLUMN IF NOT EXISTS carrybee_consignment_id text,
  ADD COLUMN IF NOT EXISTS steadfast_consignment_id text;

CREATE INDEX IF NOT EXISTS idx_orders_carrybee_consignment_id
  ON orders(carrybee_consignment_id);

CREATE INDEX IF NOT EXISTS idx_orders_steadfast_consignment_id
  ON orders(steadfast_consignment_id);
