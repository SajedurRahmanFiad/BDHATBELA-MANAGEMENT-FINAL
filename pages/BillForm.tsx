      ...
      <div className="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm space-y-6">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              ...
              >
                {selectedVendor ? (
                  <div className="flex-1 overflow-hidden">
                    ...
                    <p className="text-[10px] text-blue-600 italic truncate mt-1">{selectedVendor.address}</p>
                  </div>
              ...
              {showVendorSearch && (
                <div className="absolute top-full left-0 mt-2 w-full max-w-xs bg-white border border-gray-200 shadow-2xl rounded-2xl z-[110] p-2 overflow-hidden animate-in fade-in zoom-in-95 duration-200">
                  <div className="relative mb-2">
                    ...
                    <input autoFocus type="text" placeholder="Search business..." className="w-full pl-9 pr-4 py-2.5 bg-gray-50 border border-gray-100 rounded-xl outline-none focus:ring-2 focus:ring-emerald-500 text-sm font-medium" value={vendorSearchTerm} onChange={(e) => setVendorSearchTerm(e.target.value)} />
                  </div>
                  <div className="max-h-[220px] overflow-y-auto space-y-0.5 custom-scrollbar">
                    {db.vendors
                      ...
                      .map(v => (
                      <button key={v.id} onClick={() => { setVendorId(v.id); setShowVendorSearch(false); setVendorSearchTerm(''); }} className="w-full px-4 py-2.5 text-left hover:bg-blue-50 rounded-lg group transition-colors">
                        <p className="text-sm font-bold text-gray-800 group-hover:text-blue-700">{v.name}</p>
                        <p className="text-[10px] text-gray-400 group-hover:text-blue-600/60">{v.phone}</p>
                      </button>
                    ))}
                  </div>
                  <button onClick={() => { setShowVendorSearch(false); navigate('/vendors/new'); }} className="w-full mt-2 py-3 text-blue-600 text-[10px] font-black uppercase tracking-widest border-t border-gray-50 hover:bg-blue-50 transition-colors">+ Add New Vendor</button>
                </div>
              )}
            </div>
          </div>
          ...
          <div className="space-y-1">
            ...
            <input type="date" value={billDate} onChange={(e) => setBillDate(e.target.value)} className="w-full px-4 py-3 bg-gray-50 border border-gray-100 rounded-xl focus:ring-2 focus:ring-emerald-500 focus:bg-white transition-all cursor-pointer font-bold text-sm" />
          </div>

          <div className="space-y-1">
            <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest ml-1">Bill Number</label>
            <input type="text" value={billNumber} onChange={e => setBillNumber(e.target.value)} className="w-full px-4 py-3 bg-gray-50 border border-gray-100 rounded-xl font-mono text-blue-800 text-sm font-bold" />
          </div>
        </div>
            ...
            <tbody className="divide-y divide-gray-50">
              {items.map((item, idx) => (
                <tr key={idx} className="group hover:bg-gray-50/50">
                  ...
                  <td className="px-4 py-4 text-center">
                    <input type="number" value={item.quantity} onChange={(e) => updateQuantity(idx, parseInt(e.target.value))} className="w-16 text-center py-2 border border-gray-100 rounded-xl focus:ring-2 focus:ring-blue-500 font-bold outline-none" />
                  </td>
              ...
              <tr>
                <td colSpan={5} className="px-6 py-5 relative">
                  <div className="relative">
                    <button onClick={() => setShowProductSearch(!showProductSearch)} className="flex items-center gap-2 text-blue-600 font-black text-[10px] uppercase tracking-widest hover:bg-blue-50 px-4 py-2.5 rounded-xl border-2 border-dashed border-blue-100 transition-all">
                    ...
                    {showProductSearch && (
                      <div className="absolute top-full left-0 mt-3 w-full max-w-md bg-white border border-gray-200 shadow-2xl rounded-2xl z-[100] p-2 overflow-hidden animate-in slide-in-from-top-2 duration-200">
                        <div className="relative mb-2">
                          ...
                          <input autoFocus type="text" placeholder="Search product..." className="w-full pl-9 pr-4 py-2.5 bg-gray-50 border border-gray-100 rounded-xl outline-none focus:ring-2 focus:ring-blue-500 text-sm font-medium" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} />
                        </div>
                        <div className="max-h-[260px] overflow-y-auto space-y-0.5 custom-scrollbar">
                          {db.products.filter(p => p.name.toLowerCase().includes(searchTerm.toLowerCase())).map(p => (
                            <button key={p.id} onClick={() => addItem(p.id)} className="flex items-center gap-4 w-full px-4 py-3 text-left hover:bg-blue-50 rounded-xl group transition-all">
                              <img src={p.image} className="w-10 h-10 rounded-lg object-cover border border-gray-100 shadow-sm" />
                              <div className="flex-1 min-w-0">
                                <p className="text-sm font-bold text-gray-800 group-hover:text-blue-700 truncate">{p.name}</p>
                                <p className="text-[10px] font-bold text-blue-600/60 uppercase tracking-widest">Cost: {formatCurrency(p.purchasePrice)}</p>
                              </div>
                            </button>
                          ))}
                        </div>
        ...
        <div className="flex flex-col md:flex-row justify-between gap-12 pt-6">
          <div className="flex-1 space-y-2">
            ...
            <textarea placeholder="Bill details, vendor instructions, or delivery notes..." value={notes} onChange={(e) => setNotes(e.target.value)} className="w-full px-5 py-4 bg-gray-50 border border-gray-100 rounded-2xl h-32 focus:ring-2 focus:ring-blue-500 focus:bg-white outline-none font-medium text-sm transition-all" />
          </div>
          <div className="w-full md:w-96 space-y-4 bg-gray-50/50 p-8 rounded-[2rem] border border-gray-100">
            ...
            <div className="flex justify-between items-center text-gray-500 text-sm font-bold uppercase tracking-widest"><span>Discount</span><input type="number" value={discount} onChange={(e) => setDiscount(parseFloat(e.target.value) || 0)} className="w-24 text-right px-3 py-1.5 border border-gray-100 rounded-lg focus:ring-2 focus:ring-blue-500 font-black text-gray-900 bg-white" /></div>
            <div className="flex justify-between items-center text-gray-500 text-sm font-bold uppercase tracking-widest"><span>Shipping</span><input type="number" value={shipping} onChange={(e) => setShipping(parseFloat(e.target.value) || 0)} className="w-24 text-right px-3 py-1.5 border border-gray-100 rounded-lg focus:ring-2 focus:ring-blue-500 font-black text-gray-900 bg-white" /></div>
            <div className="pt-6 border-t-4 border-blue-100 flex justify-between items-center"><span className="text-lg font-bold text-gray-900 uppercase tracking-tighter">Total Bill</span><span className="text-3xl font-black text-blue-600">{formatCurrency(total)}</span></div>
            <button onClick={handleSave} className="w-full py-5 bg-blue-600 text-white rounded-[1.5rem] font-black text-lg shadow-xl shadow-blue-100 hover:bg-blue-700 active:scale-95 transition-all mt-4">Save Purchase Bill</button>
          </div>
        </div>
      </div>
    </div>
